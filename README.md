...
<!-- generated by `go run ./cmd/supportmatrix` — do not hand-edit between the markers -->

**Git hosts**

| | State | Notes |
|---|---|---|
| **Gitea** | ✅ proven | plain git over HTTPS — no provider code, see pkg/provider's own doc comment |
| **GitHub** | ✅ proven | nightly and on demand, never on push — a GitHub outage must not redden the build |
| **GitLab** | 🔧 code only, no CI proof | full Provider implementation, no e2e test — see #101 |
| **Bitbucket** | ❌ not implemented | — |
| **Azure DevOps** | ❌ not implemented | — |
...
|---|---|---|
| **GHCR** | ✅ proven | — |
| **ECR** | ✅ proven | OIDC, no static keys — see the workflow's role-to-assume step |
| **Harbor** | ✅ proven | stood in for by registry:2 with htpasswd — same distribution API, see #50 |
| **Docker Hub** | 🧪 configured, not yet proven | — |
| **GAR** | ❌ not implemented | — |
| **ACR** | ❌ not implemented | — |
| **Quay** | ❌ not implemented | — |
