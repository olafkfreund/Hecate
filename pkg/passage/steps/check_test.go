...
func TestCheckersAcceptValidConfiguration(t *testing.T) {
	for _, tc := range []struct {
		...
		{NewRenderHelm(), `{"chart":"repo/chart","out":"repo/r.yaml","releaseName":"api"}`},
		{NewOCIPush(nil), `{"path":"repo/rendered","repo":"ghcr.io/acme/m","tag":"1.0.0"}`},
		{NewOCIPull(nil), `{"repo":"ghcr.io/acme/m","tag":"1.0.0","out":"repo/fetched"}`},
...
func TestCheckersCatchMissingAndWrongValues(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a helm render with no release name", NewRenderHelm(), `{"chart":"repo/c","out":"r.yaml"}`, "releaseName is required"},
		{"an oci push with no tag", NewOCIPush(nil), `{"path":"p","repo":"ghcr.io/a/b"}`, "tag is required"},
		{"an oci pull with both tag and digest", NewOCIPull(nil), `{"repo":"ghcr.io/a/b","out":"o","tag":"1","digest":"sha256:x"}`, "not both"},
